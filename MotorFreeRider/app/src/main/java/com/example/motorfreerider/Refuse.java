...
public class Refuse {
    ...
    public Refuse(user _user) {
        this._user = _user;
    }

    private class HttpAsyncTask_Refuse extends AsyncTask<String, Void, String> {
        @Override
        protected String doInBackground(String... urls) {
            return POST_Refuse(urls[0],account.getId());
        }
    ...
    private String POST_Refuse(String url, String Owner) {
        String result = "false";
        try {
            String Passenger =_user.getID();
            // 1. create HttpClient
    ...
    public String refuse() throws ExecutionException, InterruptedException {
        return new HttpAsyncTask_Refuse().execute("http://140.121.197.213:8080/Servlet/ReceiveFile").get();
    }
}